def test_delete_v1_account_login_all(account_helper, prepare_user):
    ...
    response_other = account_helper.user_login(login=login, password=password)
    ...
    token_b = account_helper.dm_account_api.account_api.session.headers.get('x-dm-auth-token')    # Сравнение токенов разных сессий
    ...
    account_helper.get_current_user_info(
        ...
        status_code=401,
        error_message="Сессия на другом устройстве должна была закрыться"
    )
    # Проверка: Текущая сессия (Токен Б) должна остаться действительной
    account_helper.get_current_user_info(
        status_code=200,
        error_message="Текущая сессия должна была остаться активной"
    )
